...
//  -- 04/11/2020: implement signals
//  -- 04/21/2020: implement PGSE
//  -- 04/23/2020: replace pow with pow2, fix the bug in translateFlag
//  -- 12/14/2023: fix the mechanism of membrane permeation
//  -- 07/19/2024: seperate the (Delta, delta) and (b-value, b-vector) to accelerate the phase calculation
//  -- 08/11/2025: implement 2 populations (alive and dead cells)
//
//  Created by Hong-Hsi Lee in February, 2017.
//  Modified by Maelyne Bennasar in August 2025
//
...
__device__ void permeateE2I (const double x[], const double v[], const double &dxEX, const double xc_in[], const double &rc, const double &dxIN, const bool &translateFlag, double xt[]) {
    ...
    if (discri<=0) {                    // Does not encounter the cell membrane
        ...
        if ( (t>=dxEX) | (t<0) ) {      // Does encounter the cell membrane
            ...
            // Diffusion across the membrane needs the adjustment of the step size
            // xt = xm + (1-t/dxEX)*dxIN * v
            xt[0]=xm[0]+(1-t/dxEX)*dxIN*v[0];
            xt[1]=xm[1]+(1-t/dxEX)*dxIN*v[1];
            xt[2]=xm[2]+(1-t/dxEX)*dxIN*v[2];
        }
    }
}
...
__global__ void propagate_two_populations(
    ...
    double *NParBin, 
    double *sig, 
    double *NParICS_alive,      // New
    double *NParICS_dead,       // New
    const int TN, 
    const int NPar, 
    const double res, 
    const double stepIN, 
    const double stepEX, 
    const double probI2E_alive, // New: Probability of I2E for alive cells
    const double probE2I_alive, // New: Probability of E2I for alive cells
    const double probI2E_dead,  // New: Probability of I2E for dead cells
    const double probE2I_dead,  // New: Probability of E2I for dead cells
    const unsigned int NPix, 
    ...
    const double *zCir, 
    const double *rCir, 
    const int *cellType,        // New : cells type (0=alive, 1=dead)
    const bool *translateFlag, 
    const unsigned int *APix, 
    const double *btab,
    const double *DELdel,
    double dt){
    ...
    for (int k=idx; k<NPar; k+=stride){
        ...
        // New: to differentiate populations
        bool isInAliveCell=false;                           // true: in alive cell, false: not in alive cell
        bool isInDeadCell=false;                            // true: in dead cell, false: not in dead cell
        double current_probI2E=0, current_probE2I=0;        // Probability according to the type of cell
        ...
        for (int i=0; i<TN; i++){
            ...
            // Check if the particle is in ICS and determine cell type
            instruction1=false; instruction2=false;
            isInAliveCell=false; isInDeadCell=false;
            
            if ( a1 ) {
                xCirTmp[0]=xCir[a1-1]; xCirTmp[1]=yCir[a1-1]; xCirTmp[2]=zCir[a1-1];
                instruction1=inICS(xt,xCirTmp,rCir[a1-1],translateFlag[a1-1]);
                if (instruction1) {
                    if (cellType[a1-1] == 0) isInAliveCell = true;
                    else isInDeadCell = true;
                }
            }
            ...
            if ( a2 ) {
                ...
                if (instruction2) {
                    if (cellType[a2-1] == 0) isInAliveCell = true;
                    else isInDeadCell = true;
                }
            }
            ...
            while (iterateFlag & (ite<nite)) {
                if ( (instruction1==false) & (instruction2==false) ) {
                    ...
                    if ( (instruction[0]==false) & (instruction[1]==false) & (instruction[2]==false) & (instruction[3]==false) ) {
                        ...
                        // New: determine the probability depending on the type of cell
                        if (cellType[acell_hit-1] == 0) {
                            current_probE2I = probE2I_alive;
                        } else {
                            current_probE2I = probE2I_dead;
                        }
                        
                        vrand=curand_uniform_double(&localstate);
                        if (vrand<current_probE2I) {
                            ...
                            // New : update the type of cell in which the particule is
                            if (cellType[acell_hit-1] == 0) {
                                isInAliveCell = true;
                                isInDeadCell = false;
                            } else {
                    ...
                    if ( instruction[0] ) {
                        ...
                        // New : keep the type of cell
                        if (cellType[acell[0]-1] == 0) {
                            isInAliveCell = true;
                            isInDeadCell = false;
                        } else {
                            isInAliveCell = false;
                        ...
                        // New : determine the probability according to the type of cell
                        if (cellType[acell_hit-1] == 0) {
                            current_probI2E = probI2E_alive;
                        } else {
                            current_probI2E = probI2E_dead;
                        }
                        
                        vrand=curand_uniform_double(&localstate);
                        if (vrand<current_probI2E) {
                            ...
                            // Use xTmp to save the present position
                            xTmp[0]=xt[0]; xTmp[1]=xt[1]; xt[2]=xt[2];
            ...
            if ( (i%Tstep) ==0 ) { // Save moment tensor for dx^2 and dx^4, and signal for the b-table
                ...
                // New : count the number of particules according to the type of cell
                if ( ICSFlag ) { 
                    atomAdd(&NParICS[tidx],1); 
                    if (isInAliveCell) {
                        atomAdd(&NParICS_alive[tidx],1);
                    } else if (isInDeadCell) {
                        atomAdd(&NParICS_dead[tidx],1);
                    }
                }
...
//    int Nbvec=0;                // Number of gradient directions
    ...
    double kappa_alive=0;       // Permeability of alive cells in um/ms
    double kappa_dead=0;        // Permeability of dead cells in um/ms
    double f_alive=0.5;         // Fraction of alive cells (0.0 to 1.0)
    ...
    myfile0>>Din; myfile0>>Dex; 
    myfile0>>kappa_alive; myfile0>>kappa_dead; myfile0>>f_alive;
    myfile0>>initFlag;
    ...
    //Identification of the type of cell
    thrust::host_vector<int> cellType(NAx);

    // Option 1 read from a file
    ifstream myfile_celltype ("phantom_cellType.txt", ios::in);
    if (myfile_celltype.is_open()) {
        for (i=0; i<NAx; i++){
            myfile_celltype>>cellType[i];
        }
        myfile_celltype.close();
    } else {
        // Option 2: Random distribution depending on f_alive
        srand(time(NULL));
        for (i=0; i<NAx; i++){
            cellType[i] = (rand() / (double)RAND_MAX < f_alive) ? 0 : 1;
        }
    }
    ...
    for (i=0; i<NAx; i++){
        if (cellType[i] == 0) n_alive++;
        else n_dead++;
    }
    cout << "Alive cells: " << n_alive << " (" << (n_alive*100.0/NAx) << "%)" << endl;
    cout << "Dead cells: " << n_dead << " (" << (n_dead*100.0/NAx) << "%)" << endl;
...
//    myfile9.close();
    ...
    // Differentiate dead from alive cells
    const double probE2I_alive=(2.0/3.0*stepEX*kappa_alive/Dex)/( 1 + (2.0/3.0*stepEX*kappa_alive/Dex + 2.0/3.0*stepIN*kappa_alive/Din)/2.0 );
    const double probI2E_alive=(2.0/3.0*stepIN*kappa_alive/Din)/( 1 + (2.0/3.0*stepEX*kappa_alive/Dex + 2.0/3.0*stepIN*kappa_alive/Din)/2.0 );
    
    const double probE2I_dead=(2.0/3.0*stepEX*kappa_dead/Dex)/( 1 + (2.0/3.0*stepEX*kappa_dead/Dex + 2.0/3.0*stepIN*kappa_dead/Din)/2.0 );
    const double probI2E_dead=(2.0/3.0*stepIN*kappa_dead/Din)/( 1 + (2.0/3.0*stepEX*kappa_dead/Dex + 2.0/3.0*stepIN*kappa_dead/Din)/2.0 );
    ...
    thrust::host_vector<double> NParICS_alive(timepoints);   //New
    thrust::host_vector<double> NParICS_dead(timepoints);   //New
    for (i=0;i<timepoints*6;i++){ dx2[i]=0; }
    for (i=0;i<timepoints*15;i++){ dx4[i]=0; }
    for (i=0;i<timepoints*nbin;i++){ NParBin[i]=0; }
    for (i=0;i<timepoints;i++){ NParICS[i]=0; NParICS_alive[i]=0; NParICS_dead[i]=0;}.  //New
    ...
    thrust::device_vector<int> d_cellType=cellType;   //New
    thrust::device_vector<double> d_NParICS_alive=NParICS_alive;   //New
    thrust::device_vector<double> d_NParICS_dead=NParICS_dead;  //New
//    thrust::device_vector<double> d_TD=TD;
    
    // Parallel computation
    begin=clock();
    propagate_two_populations<<<numBlocks, blockSize>>>(devState.data().get(), 
                                        d_dx2.data().get(), 
                                        d_dx4.data().get(), 
                                        d_NParICS.data().get(), 
                                        d_NParBin.data().get(), 
                                        d_sig.data().get(), 
                                        d_NParICS_alive.data().get(),    // New
                                        d_NParICS_dead.data().get(),     // New
                                        TN, NPar, res, stepIN, stepEX, 
                                        probI2E_alive, probE2I_alive,    // New
                                        probI2E_dead, probE2I_dead,      // New
                                        NPix, Nmax, Nbtab, NDelta, initFlag, 
                                        d_xCir.data().get(), 
                                        d_yCir.data().get(), 
                                        d_zCir.data().get(), 
                                        d_rCir.data().get(), 
                                        d_cellType.data().get(),         // New
                                        d_translateFlag.data().get(), 
                                        d_APix.data().get(), 
                                        d_btab.data().get(), 
                                        d_DELdel.data().get(),
                                        dt);
    ...
    thrust::copy(d_NParICS_alive.begin(), d_NParICS_alive.end(), NParICS_alive.begin());   //New
    thrust::copy(d_NParICS_dead.begin(), d_NParICS_dead.end(), NParICS_dead.begin());   //New
    ...
    ofstream fNParICSout_alive("NParICS_alive.txt");   //File for alive cells
    ofstream fNParICSout_dead("NParICS_dead.txt");   //File for dead cells
    ...
    fNParICSout_alive.precision(15);   //New
    fNParICSout_dead.precision(15);   //New
    ...
    for (i=0; i<timepoints; i++) {
        ...
        fNParICSout_alive<<NParICS_alive[i]<<endl;  //New
        fNParICSout_dead<<NParICS_dead[i]<<endl;   //New
    }
    ...
    fNParICSout.close();
    fsigout.close();
    fNParICSout_alive.close();
    fNParICSout_dead.close();
    ...
    paraout<<Din<<endl<<Dex<<endl<<kappa_alive<<endl<<kappa_dead<<endl<<f_alive<<endl<<initFlag<<endl<<res<<endl;
    paraout.close();
    
    // Download cell population statistics
    ofstream popstatsout("population_stats.txt");
    popstatsout<<"Total cells: "<<NAx<<endl;
    popstatsout<<"Alive cells: "<<n_alive<<" ("<<(n_alive*100.0/NAx)<<"%)"<<endl;
    popstatsout<<"Dead cells: "<<n_dead<<" ("<<(n_dead*100.0/NAx)<<"%)"<<endl;
    popstatsout<<"Alive permeability: "<<kappa_alive<<endl;
    popstatsout<<"Dead permeability: "<<kappa_dead<<endl;
    popstatsout.close()
