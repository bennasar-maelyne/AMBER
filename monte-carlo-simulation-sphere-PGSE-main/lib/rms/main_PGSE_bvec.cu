...
__global__ void propagate(
    ...
    const double stepIN, 
    const double stepEX, 
    const double probI2E, 
    const double probE2I, 
...
//    myfile9.close();
    ...
    const double probE2I=(2.0/3.0*stepEX*kappa/Dex)/( 1 + (2.0/3.0*stepEX*kappa/Dex + 2.0/3.0*stepIN*kappa/Din)/2.0 );          // Probability constant from ECS to ICS
    const double probI2E=(2.0/3.0*stepIN*kappa/Din)/( 1 + (2.0/3.0*stepEX*kappa/Dex + 2.0/3.0*stepIN*kappa/Din)/2.0 );          // Probability constant from IAS to ECS
...
//    thrust::device_vector<double> d_TD=TD;
    ...
    propagate<<<numBlocks, blockSize>>>(devState.data().get(), 
                                        ...
                                        d_NParBin.data().get(), 
                                        d_sig.data().get(), 
                                        TN, NPar, res, stepIN, stepEX, probI2E, probE2I, NPix, Nmax, Nbtab, NDelta, initFlag, 
                                        d_xCir.data().get(), 
                                        d_yCir.data().get(), 
                                        d_zCir.data().get(), 
                                        d_rCir.data().get(), 
