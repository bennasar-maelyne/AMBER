"""
T2 sensitivity analysis for Level 2 optimisation.

Reuses the LHS signals already stored in opt_results.npz (no need to re-run
opt_engine.py) and sweeps T2 over a configurable range.

Generates:
  fig_t2_heatmaps.pdf  — CNR heatmaps for each T2 value, one column per T2,
                          one row per phenotype pair
  fig_t2_optimum.pdf   — optimal (b*, TD*, peak CNR) as phenotype × phenotype
                          matrices, one row per T2 value + Level 1 reference
"""
...
# ── Config ─────────────────────────────────────────────────────────────────────
T2_VALUES = [100, 200, 500, 1000]   # ms — values to sweep
SNR0      = 40.0                            # fixed (calibrated from noise maps)

THIS_DIR    = os.path.dirname(os.path.abspath(__file__))
NPZ_PATH    = os.path.join(THIS_DIR, "results", "opt_results.npz")
OUT_DIR     = os.path.join(THIS_DIR, "Figures", "t2_sensitivity")
...
# Reload Level 1 signals and CNR for each pair
sigs   = {}
cnr_l1 = {}
for pA, pB in pairs:
    key = f"{pA}_vs_{pB}"
...
# ── Compute Level 2 metrics for every T2 ──────────────────────────────────────
print(f"Computing metrics for {len(T2_VALUES)} T2 values x {len(pairs)} pairs...")
metrics = {}   # t2 -> pair_key -> CNR array (N_TD, N_BVAL)
for t2 in T2_VALUES:
    metrics[t2] = {}
    for pA, pB in pairs:
        key = f"{pA}_vs_{pB}"
...
# =============================================================================
# Fig 1 — CNR heatmaps: rows = pairs, columns = T2 values
# =============================================================================
n_t2   = len(T2_VALUES)
n_pairs = len(pairs)

# Global colour scale: max CNR across all T2 and pairs
vmax = max(np.nanmax(metrics[t2][f"{pA}_vs_{pB}"])
           for t2 in T2_VALUES for pA, pB in pairs)

fig, axes = plt.subplots(n_pairs, n_t2,
                         figsize=(2.8 * n_t2, 2.6 * n_pairs),
                         squeeze=False)

for row, (pA, pB) in enumerate(pairs):
    key = f"{pA}_vs_{pB}"
    for col, t2 in enumerate(T2_VALUES):
        ax  = axes[row, col]
        CNR = metrics[t2][key]
        ...
        if row == 0:
            ax.set_title(f"T₂ = {t2} ms", fontsize=9, fontweight="bold")
        if col == 0:
...
fig.suptitle(f"CNR heatmaps vs T₂  (SNR₀={SNR0:.0f}, δ={delta:.0f} ms)\n"
             "cyan star = optimal (b, TD)", fontsize=11, y=1.01)

out1 = os.path.join(OUT_DIR, "fig_t2_heatmaps.png")
...
#          One row per T2 value + last row = Level 1 reference (no noise).
# =============================================================================
n_pheno      = len(pheno_names)
short_labels = [label_map[n] for n in pheno_names]

# ── Build matrices for a given sweep parameter ────────────────────────────────
def make_matrices(metrics_dict, param_key):
    ...
    for pA, pB in pairs:
        ...
        CNR = metrics_dict[param_key][f"{pA}_vs_{pB}"]
        best_ti, best_bi = np.unravel_index(np.nanargmax(CNR), CNR.shape)
...
all_rows = [(t2, *make_matrices(metrics, t2)) for t2 in T2_VALUES]
all_rows.append(("L1 (no noise)", *make_l1_matrices()))

# ── Global colour scales per metric ───────────────────────────────────────────
vmin_b,  vmax_b   = np.nanmin([r[1] for r in all_rows]), np.nanmax([r[1] for r in all_rows])
vmin_td, vmax_td  = np.nanmin([r[2] for r in all_rows]), np.nanmax([r[2] for r in all_rows])
vmin_cnr = 0;      vmax_cnr = np.nanmax([r[3] for r in all_rows])
...
for row, (row_label, b_mat, td_mat, cnr_mat) in enumerate(all_rows):
    ...
    for col, (col_title, vmin, vmax, cmap_name, fmt) in enumerate(col_configs):
        ...
        if col == 0:
            ...
            row_title = f"T₂ = {row_label} ms" if not is_l1 else row_label
...
fig2.suptitle(
    f"Optimal (b*, TD*, CNR) per phenotype pair — sensitivity to T₂\n"
    f"SNR₀ = {SNR0:.0f}  |  δ = {delta:.0f} ms  "
...
out2 = os.path.join(OUT_DIR, "fig_t2_optimum.png")
