...
The code to generate a new lookup table is in the repertory monte-carlo-simulation-sphere-PGSE-main. The first step is to generate files with different configuration for the voxel with cellsize_sphere_PGSE_with_dictionary_creation_V*.mlx. Then, the MC simulation requires a GPU to run the main_PGSE_bvec_cuda. Two options are implemented, either running on a cluster with the slurm or runing incrementally with the bash code. Finally, extraction of the LUt and sensitivy analysis are done when executing LUT_extraction.mlx or LUT_extraction_necrosis.mlx (for two populations of cells).